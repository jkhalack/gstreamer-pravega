...
fn test_timestampcvt() {
    ...
    push_and_validate(&mut h, 0 * gst::ClockTime::MSECOND, None);
    push_and_validate(&mut h, 1000 * gst::ClockTime::MSECOND, None);
    println!("No PTS.");
    push_and_validate(&mut h, ClockTime::NONE, None);
    println!("Key frame with multiple buffers at same PTS.");
    push_and_validate(&mut h, first_input_pts + 0 * gst::ClockTime::MSECOND, Some(first_expected_pts + 0 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, first_input_pts + 0 * gst::ClockTime::MSECOND, Some(first_expected_pts + 0 * gst::ClockTime::MSECOND));
    println!("Delta frames.");
    push_and_validate(&mut h, first_input_pts + 50 * gst::ClockTime::MSECOND, Some(first_expected_pts + 50 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, first_input_pts + 100 * gst::ClockTime::MSECOND, Some(first_expected_pts + 100 * gst::ClockTime::MSECOND));
    println!("Large jump forward.");
    push_and_validate(&mut h, first_input_pts + 1000 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1000 * gst::ClockTime::MSECOND));
    println!("Decreasing PTS.");
    push_and_validate(&mut h, first_input_pts + 500 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1015 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, first_input_pts + 500 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1015 * gst::ClockTime::MSECOND));
    println!("Next frame but still decreasing.");
    push_and_validate(&mut h, first_input_pts + 550 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1030 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, first_input_pts + 550 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1030 * gst::ClockTime::MSECOND));
    println!("Back to PTS before decrease.");
    push_and_validate(&mut h, first_input_pts + 1000 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1045 * gst::ClockTime::MSECOND));
    println!("Back to normal.");
    push_and_validate(&mut h, first_input_pts + 1050 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1050 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, first_input_pts + 1050 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1050 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, first_input_pts + 1100 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1100 * gst::ClockTime::MSECOND));
    println!("No PTS, part 2.");
    push_and_validate(&mut h, ClockTime::NONE, None);
    println!("Back to normal, part 2.");
    push_and_validate(&mut h, first_input_pts + 1150 * gst::ClockTime::MSECOND, Some(first_expected_pts + 1150 * gst::ClockTime::MSECOND));
...
fn test_timestampcvt_start_at_zero() {
    ...
    let t0 = push_and_get_pts(&mut h, 0 * gst::ClockTime::MSECOND);
    assert!(t0 > expected_t0 - 60 * gst::ClockTime::SECOND);
    assert!(t0 < expected_t0 + 60 * gst::ClockTime::SECOND);
    push_and_validate(&mut h, 1000 * gst::ClockTime::MSECOND, Some(t0 + 1000 * gst::ClockTime::MSECOND));
    push_and_validate(&mut h, 2000 * gst::ClockTime::MSECOND, Some(t0 + 2000 * gst::ClockTime::MSECOND));
...
fn test_timestampcvt_start_fixed_time() {
    ...
    push_and_validate(&mut h, 10000 * gst::ClockTime::MSECOND, Some(expected_t0));
    push_and_validate(&mut h, 10100 * gst::ClockTime::MSECOND, Some(expected_t0 + 100 * gst::ClockTime::MSECOND));
